...
const std::array<int, 8> Connect4Game::colValues = { {0,1,2,3,4,3,2,1} };
...
{
	return won || getMoveCounter() == SIZE;
}
...
void Connect4Game::getMoves_impl(MovesPerPositionType* moves) const
{
	for (int move = 1; move <= 7; move++)
	{
		if (isPlayable(move))
		{
			moves->addMove(move, colValues[move]);
		}
	}
}
